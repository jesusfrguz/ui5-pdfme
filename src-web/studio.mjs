...
export class WebPdfTemplateStudio {
  ...
  render() {
    ensureStyles();
    const t = labels[this.language];
    const catalogButton = this.templateStore?.repositories.length ? `<button type="button" data-action="openTemplateCatalog" class="pdfme-web-button" title="${t.templates}">${icons.templates}<span class="pdfme-web-button-label">${t.templates}</span></button>` : "";
