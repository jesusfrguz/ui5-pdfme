sap.ui.define([], function () {
  ...
  function normalize(input, repositoryId) {
    ...
    var record = {
      ...
      updatedAt: value(raw, ["updatedAt", "UpdatedAt", "modifiedAt", "ModifiedAt"], null),
      createdAt: value(raw, ["createdAt", "CreatedAt"], null),
  ...
  function request(source, context, url, options) {
    ...
    settings.headers = Object.assign({}, source.headers || {}, settings.body ? { "content-type": "application/json" } : {}, settings.headers || {});
  ...
  var odataProvider = {
    list: function (source, query, context) {
      ...
      if (isV4(model)) {
        var binding = model.bindList(source.path, source.context, source.sorters, source.filters, source.parameters || source.query || {});
        var pageSize = source.pageSize || 100;
        var load = function (skip, records) {
          return binding.requestContexts(skip, pageSize).then(function (contexts) {
            ...
            var combined = records.concat(items);
            return items.length === pageSize && combined.length < (source.maxRecords || 10000) ? load(skip + items.length, combined) : combined;
          });
        };
      ...
      return new Promise(function (resolve, reject) {
        model.read(source.path, {
          ...
          success: function (data) { resolve(filter((data.results || [data]).map(function (record) { return normalize(record, source.id); }), query)); },
          error: reject
        });
      });
    },
    ...
    save: function (source, input, context) {
      ...
      var create = !record.id;
      record.id = record.id || id();
      var entity = odataEntity(record);
      if (!model) {
        var httpUrl = create ? source.url : source.url.replace(/\/$/, "") + "('" + encodeURIComponent(record.id) + "')";
        return request(source, context, httpUrl, { method: create ? source.createMethod || "POST" : source.updateMethod || "PATCH", body: JSON.stringify(entity) })
          .then(function (saved) { return normalize(saved && saved.d || saved || entity, source.id); });
      }
      if (isV4(model)) {
        if (create) {
        ...
        return binding.requestObject().then(function () {
          Object.keys(entity).forEach(function (field) { if (entity[field] !== undefined) { binding.getBoundContext().setProperty(field, entity[field]); } });
          return source.updateGroupId && model.submitBatch ? model.submitBatch(source.updateGroupId) : null;
        }).then(function () { return record; });
      }
      return new Promise(function (resolve, reject) {
        var settings = { success: function (saved) { resolve(normalize(saved || entity, source.id)); }, error: reject };
